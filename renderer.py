...
def compute_impact(donation_gbp: float, parish: Optional[str]) -> Impact:
    ...
    farmers = FIXED_FAMILIES                # new update to the previous code
    acres = FIXED_ACRES
...
def render_impact_menu_png(donation_gbp: float, parish: Optional[str] = None) -> Tuple[bytes, Impact]:
    ...
    # Headline
    headline = (
    f"Your {fmt_currency(imp.donation_gbp, currency)} restores "
    f"farmers’ income, livelihoods and hope for communities in "
    f"{imp.parish}, Jamaica"
    )
    draw_text_block(
        draw,
    ...
    cols = [
        ...
        ("Production restored", fmt_acres(imp.acres_restored)),
